...
def run_test_largest_number():
    ...
    expected = 48
    answer = largest_number(([24,32,17], [-2],[], [48]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    # ------------------------------------------------------------------
    maxK = find_first_non_empty_seq(seq_seq)
    maxJ = 0
    if maxK != None:
...
def find_first_non_empty_seq(seq_seq):
    for k in range(len(seq_seq)):
        if len(seq_seq[k]) > 0:
            return k
    return None
...
def run_test_largest_negative_number():
    ...
    print('-------------------------------------------------')
    expected = -4
    answer = largest_negative_number([[24, 32, 17,-6], [-4], [], [48]])
    print('Expected and actual are:', expected, answer)

    expected = -2
    answer = largest_negative_number([[], [-24,-3,0,8], [19,6,-2], [48,-3]])
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    # ------------------------------------------------------------------
    check = find_first_negative(seq_seq)
    maxK = 0
    maxJ = 0
    if check != None:
        maxK = check[0]
    ...
    if maxK != None:
        for k in range(len(seq_seq)):
            if (len(seq_seq[k]) > 0):
                for j in range(len(seq_seq[k])):
                    if seq_seq[k][j] > seq_seq[maxK][maxJ] and seq_seq[k][j] < 0:
                        maxJ = j
                        maxK = k
    if maxK == 0 and maxJ == 0:
        return None
...
def find_first_negative(seq_seq):
    #Returns the indicies of the first negative in list format
    for k in range(len(seq_seq)):
        if len(seq_seq[k]) > 0:
            for j in range(len(seq_seq[k])):
                if seq_seq[k][j] < 0:
